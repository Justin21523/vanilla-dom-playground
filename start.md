太好了，這個專案超適合你現在的學習目標。先給你一個清楚、可落地的規劃（中文說明為主），再附上一份你可以直接丟給 LLMProvider Tooling 的「英文專案說明文字」（不幫你主動建檔，純貼上用）。最後我也幫你想好 GitHub repo 名稱與短描述。
...
# 交給 LLMProvider Tooling 的英文說明（可直接貼過去）
