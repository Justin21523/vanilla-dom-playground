...
2. 在瀏覽器開啟 `http://localhost:8000/learn.html`
3. 選擇感興趣的遊戲、教程或項目開始學習！

### 🛠️ 構建模式（進階用戶）

- 開啟 `index.html` 使用可視化 DOM 構建器
- 拖拽元素、設定屬性、綁定事件、配置動畫
- 即時生成可運行的 HTML/CSS/JS 代碼
...
- [x] **階段 5** - 成就系統、進度追蹤、排行榜（~5,029 行代碼）✅
  - 30 個成就定義（遊戲 10 + 教程 6 + 項目 5 + 綜合 9）
  - 完整的遊戲化系統（成就、排行榜、進度追蹤）
  - 動畫彈窗 + 音效系統
  - 已整合到所有 12 個學習模塊
...
- 🎨 特效: 1 個粒子系統（6 種特效）
- 🏆 成就系統: 30 個成就、4 種稀有度、進度追蹤、排行榜
- 📄 測試頁面: 14 個獨立測試頁面（含成就系統測試）

**成就系統**:
- 30 個可解鎖成就（總計 990 點）
- 4 種稀有度等級（普通、稀有、史詩、傳說）
- 完整的進度追蹤和數據持久化
- 動畫彈窗 + Web Audio API 音效
